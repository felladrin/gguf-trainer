...
2. **`--resume` requires an exact architecture match.** `--hidden`, `--layers`, `--head-dim`,
   `--window`, `--rope-base` (gemma3 also `--rope-base-local`), `--rms-eps`, `--max-seq` and the
   vocab must equal the checkpoint's. A mismatch aborts before any compute and names the field.
   `inspect` prints the correct flags.
