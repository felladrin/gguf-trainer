...
| OOM at long context                                                                          | the activation pool, ~2.3 MB per token in flight (lever 3)            | add `--recompute` first (4.6x less pool and faster, lever 20), then `--reclaim` (lever 3b), then lower `--seq-len` or `--batch` |
