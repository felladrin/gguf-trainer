...
| `no GPU adapter found, and training needs one`                                                                            | Deno is running, but the machine has no usable GPU                                                                                                                  | a refusal, not a gap: the CPU loop is the correctness oracle and is orders of magnitude short at a real size. Use `transformers` with `peft` for CPU training: `docs/optimization.md` lever 36                                                                                                                                                                                 |
...
- Any new autograd op needs a finite-difference gradient check in `tests/gradcheck.ts`, and any new
  ...
  Node 22.6 and 24, and `deno task test:bun`, which CI runs on the current Bun with
  auto-install off.
