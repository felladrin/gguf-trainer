...
Training runs on **WebGPU** (WGSL compute shaders, forward and backward). There is no CPU
execution path: every command that computes needs a GPU adapter.
...
| change a kernel or chase throughput                | [docs/performance.md](docs/performance.md), "Ruled out" FIRST: it lists what has already been measured and rejected                                                             |
| measure anything on this box                       | [docs/performance.md](docs/performance.md), "Measuring on a shared box": the machine has other tenants and they are worth 2x, so a number without a recorded control is not one |
| add or move a guard, or chase a wrong result       | [docs/correctness.md](docs/correctness.md): the guards, gates and file formats, each with the failure it was written after                                                      |
| understand why the engine is shaped this way       | [docs/design.md](docs/design.md)                                                                                                                                                |
| pick a corpus or a token budget                    | [docs/evaluation.md](docs/evaluation.md), "The ceiling is tokens per parameter"; for model size, Hardware reality below                                                         |
| judge a checkpoint, or question a published number | [docs/evaluation.md](docs/evaluation.md) for the number and the instrument; [docs/notes/](docs/notes/) for the run that produced it, point-in-time and not maintained           |
...
2. **`--resume` requires an exact architecture match.** Every field the resume gate covers must
   ...
   whose parameter count would still match well enough to be accepted.
...
Under a minute. Trains a tiny model on the GPU, exports f16/q8_0/q4_0, re-parses each. If this
fails, nothing else will work; if it passes, later failures are about data, flags or hardware.
...
`eval-loss`, `eval-choice` and `generate` freeze every parameter before the first forward
(`freezeForScoring`). Nothing there runs backward, and on the GPU an unfrozen parameter costs a
gradient buffer allocated per run and copied back to the host on every `sync()`: 14.8% of
`eval-loss` wall clock on a 293M checkpoint and 39.0% of `generate`'s, which syncs per token.
...
and was the worse model on every other axis, benchmarks included (`docs/evaluation.md`,
"Picking a checkpoint").

Before trusting any absolute loss from a DOWNLOADED checkpoint, score one file with llama.cpp too.
Every suite here is a self-consistency check (GPU against the reference implementation, analytic
gradients against finite differences, export against re-import), so a forward pass that disagrees
with llama.cpp passes all of them. That is how the `llama` RoPE row order stayed wrong until a
fine-tune opened at loss 6.4 (`docs/correctness.md`). Repeated text is the sharpest probe: a correct model scores near zero perplexity on a
...
| `no GPU adapter found, and <command> needs one`                                                                           | the machine has no usable GPU adapter                                                                                                                               | there is no CPU execution path, by decision rather than omission. For training on a CPU, use `transformers` with `peft`: readme.md, "If you only have a CPU"                                                                                                                                                                                                                                               |
| `parity probe failed`                                                                                                     | the backend disagrees with the reference at init                                                                                                                    | a real bug; stop and report it, do not train through it                                                                                                                                                                                                                                                                                                                                                    |
...
| a loss far worse than the checkpoint deserves, on a model that still generates readable text                              | this engine and llama.cpp disagree about the forward pass                                                                                                           | score one file with both before blaming the corpus: `docs/correctness.md`, "The four instruments"                                                                                                                                                                                                                                                                                                          |
| OOM at long context                                                                                                       | the activation pool, ~2.3 MB per token in flight                                                                                                                    | add `--recompute` first (activations ~12.1 GB to ~0.8 GB measured alongside `--reclaim`, not instead of it; faster on this host-bound step, expect the textbook ~30% slower on a GPU-bound one), then `--reclaim`, then lower `--seq-len` or `--batch`                                                                                                                                                     |
| `GPU storage buffer of N MiB exceeds this device's limit`                                                                 | a `[seq-len, vocab]` logits buffer past `maxStorageBufferBindingSize`                                                                                               | `--loss-chunk 8192`, taken by `pretrain`, `finetune`, `eval-loss` and `eval-choice`; `generate` still builds dense logits                                                                                                                                                                                                                                                                                  |
| `an item's stem rendered to nothing` / `an item's choice rendered to nothing`                                             | an `eval-choice` task's render produced an empty half for some item                                                                                                 | a dataset or render bug, not a model one: the pair is printed. Raised before the first forward rather than hours into the run: `docs/correctness.md`, "Catch it when the file opens"                                                                                                                                                                                                                       |
| `a choice takes N tokens and needs one more of context`, `a choice rendered to 0 tokens`, `the stem rendered to 0 tokens` | an `eval-choice` option, or the stem it follows, cannot be scored in this model's context                                                                           | for the first, score a checkpoint with a longer context; `--shots` does not reach it, the preamble is trimmed from the left and only the choice itself has to fit. The two zero-token messages are a render or dataset problem, not a context one: `docs/correctness.md`, "Numerical forms that hide a defect"                                                                                             |
| `GGUF tensor "X" has dims [...], but this model wants [...]`                                                              | the checkpoint's stored tensor dims disagree with the shape its metadata implies                                                                                    | for a file this repo wrote, corruption: re-export it. For a foreign one, check the keys defaulted when absent, since a wrong default changes the shape allocated: `attention.key_length` falls back to `embedding_length / head_count` and `vocab_size` to the token list's length. llama.cpp's `check_tensor_dims` refuses the same mismatch: `docs/correctness.md`, "Numerical forms that hide a defect" |
| `dequantize: N elements of ggml type T need B bytes, got C`                                                               | a truncated or mis-offset tensor in the file                                                                                                                        | re-download or re-export the checkpoint. q4_0 used to decode the missing bytes as a plausible -8 x scale rather than failing: `docs/correctness.md`, "Numerical forms that hide a defect"                                                                                                                                                                                                                  |
| `<file>: token N at position M is outside [0,V)`                                                                          | the `.tokens` file was built with a different vocab than the checkpoint, or with a different id width                                                               | retokenize the corpus with the checkpoint's own tokenizer; invariant 1. This is the same mistake the runtime id and target rows further down catch, found when the file opens rather than on the window that holds it. The stamp rows between them catch the half of that mistake no range check can see: `docs/correctness.md`, "Catch it when the file opens"                                            |
| `<file> was tokenized with a different tokenizer: vocab A against the current B`                                          | a `.tokens` file survived a tokenizer change: `pretrain` reuses an existing one, and `sharedTokenizer` retrains the vocab whenever the `.tokenizer.json` is missing | delete the `.tokens` and let the run rebuild it, or point `--data` at a matching file. The row above cannot catch this: a stale file's ids are all legal ids. `docs/correctness.md`, "Catch it when the file opens"                                                                                                                                                                                        |
| `<file> was tokenized with a different tokenizer: the same V tokens but different merges or specials`                     | same as above, in the case a vocab-size check misses entirely: the vocab is the same size and retokenizes the corpus differently                                    | as above; the stamp hashes the whole exported tokenizer, not its size. `docs/correctness.md`, "Catch it when the file opens"                                                                                                                                                                                                                                                                               |
| `<file> was written A bytes per token and is about to be read as B`                                                       | the vocab crossed the u16 ceiling in one direction or the other, so `tokenBytes` picks a different width than the file was written with                             | as above. `diskTokenSource`'s size check is only `% bytesPerToken`, so a 4-byte file read as 2-byte passes it, doubles the count and splits each id into a low and a high half. `docs/correctness.md`, "Catch it when the file opens"                                                                                                                                                                      |
| `<file>.id is not readable JSON`                                                                                          | the stamp beside a `.tokens` file was truncated or hand-edited                                                                                                      | delete the `.id` and rebuild the `.tokens`; an unreadable stamp is refused rather than treated as absent. `docs/correctness.md`, "Catch it when the file opens"                                                                                                                                                                                                                                            |
| `Tokenizer match: <file> predates the .id stamp, so it cannot be checked`                                                 | the `.tokens` file was written before the stamp existed                                                                                                             | not an error: the run proceeds. Rebuild the file to get the check. Every file written from here on is stamped; the ones already on disk are as unguarded as they were. `docs/correctness.md`, "Catch it when the file opens"                                                                                                                                                                               |
| `Token file: <file> predates the .id stamp, so its id width cannot be checked against this checkpoint`                    | the same, from `eval-loss`, which checks the id width and not the tokenizer                                                                                         | not an error: the run proceeds. `eval-loss` has no tokenizer it can compare, since a GGUF-derived export is not guaranteed equal to a json-derived one. `docs/correctness.md`, "Catch it when the file opens"                                                                                                                                                                                              |
| `<file> is N bytes and its stamp was written for M`                                                                       | something rewrote or truncated the `.tokens` without restamping it                                                                                                  | rebuild both. Every writer here drops the stamp before writing, so this means the file changed by some other route: an external truncation, an interrupted copy, a restore from backup, or an `.id` moved beside a different file. `docs/correctness.md`, "Catch it when the file opens"                                                                                                                   |
| `id N at position M is not an integer in [0,V)`                                                                           | the `.tokens` file was built with a different vocab than the checkpoint, caught on the input side                                                                   | retokenize the corpus with the checkpoint's own tokenizer; invariant 1. This fires one step before the target check, since the inputs reach the embedding table first. Before it, the reference returned a NaN row and the GPU a row of zeros: `docs/correctness.md`, "Validate above the backend dispatch"                                                                                                |
| `teacher id N at slot J of row T is not an integer in [0,V)`                                                              | the KL-anchor teacher file was built against a different vocab than the checkpoint                                                                                  | rebuild the teacher logits with the checkpoint's own tokenizer. Before this check the reference refused and the GPU did not: `docs/correctness.md`, "Validate above the backend dispatch"                                                                                                                                                                                                                  |
| `target N at position M is not -1 (ignore) or an integer in [0,V)`                                                        | the `.tokens` file was built with a different vocab than the checkpoint                                                                                             | retokenize the corpus with the checkpoint's own tokenizer; invariant 1. Before this check both backends read the next row's logits and reported a plausible loss rather than stopping: `docs/correctness.md`, "Validate above the backend dispatch"                                                                                                                                                        |
...
`docs/performance.md`, "Where the step goes"). Run `bench` on the machine in front of you before planning around
any of these.
...
Deno ships WebGPU natively and is the only runtime this repo supports.
...
tests/              gradcheck (finite differences), gpu-parity (kernels vs the reference), pure-logic checks
docs/               design, performance, correctness, evaluation, how to add an architecture
...
Before optimizing a kernel, read the ruled-out table in `docs/performance.md`. It records what was tried, measured and abandoned, with the numbers: f16 compute (0.98x), f16 storage
...
- Any new autograd op needs a finite-difference gradient check in `tests/gradcheck.ts`, and any new
  WGSL kernel needs a parity check in `tests/gpu-parity.ts`. A kernel that declares
  ...
  nothing else will tell you. `deno task test` must pass; it type-checks the tree first
  (`deno task check`) because `deno run` does not.
- The engine stays dependency-free: the two npm packages belong to data fetching and chat
  templating, and must not reach `src/model/`, `src/arch/`, `src/train/`, `src/gguf/`,
  `src/tokenizer/` or `src/export/`.
