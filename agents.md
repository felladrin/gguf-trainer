...
| `no GPU adapter found, and training needs one`                                                                            | Deno is running, but the machine has no usable GPU                                                    | a refusal, not a gap: the CPU loop exists and is the correctness oracle, and is roughly four orders of magnitude short at a real size (15.8 tok/s at 6M against the GPU's 786, 1.6 at 32M). Use `transformers` with `peft` for CPU training: `docs/optimization.md` lever 36                                                                                                   |
