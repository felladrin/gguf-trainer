...
| `a choice takes N tokens and needs one more of context`, `a choice rendered to 0 tokens`, `the stem rendered to 0 tokens` | an `eval-choice` option, or the stem it follows, cannot be scored in this model's context | score a checkpoint with a longer context; `--shots` does not reach this, the preamble is trimmed from the left and only the choice itself has to fit: `docs/optimization.md` lever 24                                                                                       |
