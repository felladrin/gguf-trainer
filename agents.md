...
| `GGUF tensor "X" has dims [...], but this model wants [...]`                                                              | the checkpoint's stored tensor dims disagree with the shape its metadata implies                  | for a file this repo wrote, corruption: re-export it. For a foreign one, the common benign case is an embedding padded wider than the declared vocab, which llama.cpp tolerates because it sizes from the tensor and this repo sizes from the metadata: `docs/optimization.md` lever 33 |
