...
| measure anything on this box                 | [docs/optimization.md](docs/optimization.md) lever 47: the machine has other tenants and they are worth 2x, so a number without a recorded control is not one             |
...
| `no GPU adapter found, and training needs one`                                                                            | Deno is running, but the machine has no usable GPU                                                                                                                  | a refusal, not a gap: the CPU loop is the correctness oracle and is orders of magnitude short at a real size. Use `transformers` with `peft` for CPU training: `docs/correctness.md` lever 36                                                                                                                                                                                 |
...
- Any new autograd op needs a finite-difference gradient check in `tests/gradcheck.ts`, and any new
  ...
  first (`deno task check`) because `deno run` does not, `deno task test:node`, which CI runs on
  Node 22.6 and 24, and `deno task test:bun`, which CI runs on Bun 1.4.2 with
  auto-install off.
