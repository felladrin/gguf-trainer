...
| `id N at position M is not an integer in [0,V)`                                                                           | the same, seen one step earlier: the inputs reach the embedding table before the targets reach the loss | retokenize the corpus with the checkpoint's own tokenizer; invariant 1. Before this check the CPU returned a NaN row and the GPU a row of zeros: `docs/optimization.md` lever 29                                                                                                   |
