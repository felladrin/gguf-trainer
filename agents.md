...
| `<file>: token N at position M is outside [0,V)`                                                                          | the `.tokens` file was built with a different vocab than the checkpoint, or with a different id width                                                               | retokenize the corpus with the checkpoint's own tokenizer; invariant 1. This is the same mistake the two rows below catch, found when the file opens rather than on the window that holds it: `docs/optimization.md` lever 34                                                                                                                                                  |
