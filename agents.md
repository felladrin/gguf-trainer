...
| `target N at position M is outside [0,V)`                                                                                 | the `.tokens` file was built with a different vocab than the checkpoint                   | retokenize the corpus with the checkpoint's own tokenizer; invariant 1. Before this check the CPU read the next row's logits and reported a plausible loss: `docs/optimization.md` lever 26                                                                                        |
