...
| `<file>: token N at position M is outside [0,V)`                                                                          | the `.tokens` file was built with a different vocab than the checkpoint, or with a different id width                                                               | retokenize the corpus with the checkpoint's own tokenizer; invariant 1. This is the same mistake the runtime id and target rows further down catch, found when the file opens rather than on the window that holds it. The six stamp rows between them catch the half of that mistake no range check can see: `docs/optimization.md` levers 34 and 38                          |
...
| `<file> is N bytes and its stamp was written for M`                                                                       | something rewrote or truncated the `.tokens` without restamping it                                                                                                  | rebuild both. The stamp carries the byte size so a stale stamp beside a rewritten file cannot report a false ok. `docs/optimization.md` lever 38                                                                                                                                                                                                                               |
