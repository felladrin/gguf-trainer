...
| `a choice takes N tokens and needs one more of context`                                      | one candidate answer is at least as long as the model's whole declared context | score a checkpoint with a longer context; `--shots` does not reach this, the preamble is trimmed from the left and only the choice itself has to fit: `docs/optimization.md` lever 24                                                                                       |
