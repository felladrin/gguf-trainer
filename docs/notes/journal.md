...
> **Superseded. Read this as a diary, not as documentation.** It was written during the Minueza-3
> run and describes the repository as it was then: a web UI that has since been deleted, `deno task`
> names that no longer exist, `examples/…` scripts replaced by CLI subcommands, and a claim that
> gemma3 is the only architecture (there are three now). For what is true today, read
> [agents.md](../../agents.md). What survives here is the measurements and the reasoning that
> produced them.
...
train/inference mismatch. Key facts (see `docs/design.md`):
...
- **Gemma3 arch is implemented and validated end-to-end in llama.cpp (build 9850):** loads, reports
  ...
  `export-gguf.ts`. New ops: `gelu` (tanh-approx, for GeGLU) + `scale` (const-mul, for the √hidden
...
1. PRETRAIN a base LM (no chat template, full-sequence loss) on unlabeled English, with
   `examples/pretrain.ts`.
2. INSTRUCT SFT (ChatML) on an instruct set (`HuggingFaceTB/smol-smoltalk`, built for small models)
   with the corpus built by `examples/prepare_instruct.ts` (2026-08-05): all 460,341 conversations,
   **408.2M tokens** (887 tok/convo) plus `instruct.mask`, an assistant-only supervision mask with
   295.2M tokens (72.3%) supervised, both u16 and streamable via `diskTokenSource`. It reuses
   `blend.tokenizer.json` VERBATIM (aborts if any curriculum special is missing: the frozen-vocab
   rule below) and renders through the exporter's `DEFAULT_CHAT_TEMPLATE` with `@huggingface/jinja`,
   so training text == inference text. The written `instruct.tokenizer.json` differs from the base's
   in one field: `eosId` moves to `<|im_end|>` (chat stop token); vocab and merges are untouched, so
   the embeddings stay compatible.
...
`<think>`/`</think>`, and the six `<tool_call>`/`<tools>`/`<tool_response>` tags). ONE shared
tokenizer carries all of them: for Phase A that is `examples/blend.tokenizer.json`, **vocab 32768**,
built by `pretokenize.ts` with the `curriculum` specials set (pass that word as its 5th arg, or the
later stages have no tokens to use). Pretraining never emits them (raw text), so their embedding
rows sit at init until their stage's data first uses them (a handful of dormant rows, ~free). Every
later stage copies that same file to its own prefix BEFORE pretokenizing: `pretokenize.ts` reuses a
tokenizer that already sits at the output prefix, and trains a fresh incompatible one if it does
not.
...
break parsing). Done in `tokenTypes()` (export-gguf.ts) and `tokenizerFromGGUF()` (recovers both
...
**Launching detached.** Use deno's absolute path (`~/.deno/bin/deno`, spelled out): a bare
`env deno` under `setsid` has no `~/.deno/bin` on PATH. A `bash -lc "..."` login-shell wrapper works
too. Detached run:
`cd ~/gguf-trainer && setsid nohup ~/.deno/bin/deno run -A --unstable-webgpu examples/pretrain.ts
<corpus> 512 6 20000 512 16 0.01 > pretrain.log 2>&1 </dev/null &`.
...
- **Validation harnesses** (the gate for all future op/kernel work):
  ...
  - `tests/gpu-parity.ts`: GPU-vs-CPU forward and backward parity per op, finite differences run
    ...
    grad-norm clip), and sync() fence verification. 33 cases total. [The fence check is now the
    opposite check, #87.] Skips cleanly where WebGPU is
    unavailable.
...
staging list from the reads and the touched externals, so with nothing to stage and nothing
profiling it awaits nothing, and what makes the recycle safe is queue ordering. Lever 41.]
...
(`muon-gpu.ts`) and AdamW on the aux group (`adamw-gpu.ts`), so weights, moments, and gradients
...
Newton–Schulz now runs entirely on the GPU. New file `src/backend/muon-gpu.ts` holds `MuonGpu`:
...
Gate: trajectory-parity test in `tests/gpu-parity.ts` (`muonTrajectoryParity`) runs 4 steps of CPU
...
Gate: `tests/gpu-parity.ts` runs both paths explicitly at T=67/130/193 (each shape tested with
...
- **Resume-loader robustness.** `tokenizerFromGGUF` recovers control/special tokens from
  ...
  out of scope by design, since this is a train-from-scratch project; the loader resumes GGUFs this
...
All below are committed and gated by `tests/gpu-parity.ts`. Measured on an M1 Max and an AMD Strix
...
1. **GGUF loadability is a contract.** The `gemma3` tensor names and metadata keys in
   `src/export/export-gguf.ts` are what `llama.cpp` expects. If you touch them, re-validate the
...
2. **WSD learning-rate schedule** (warmup → stable → linear cooldown): done.
   ...
   trajectory) in `tests/gpu-parity.ts` and a shape self-check in `tests/gradcheck.ts`. Left off the
...
3. **MuonClip / attention-logit clipping**: done, adapted for QK-norm. `src/train/qk-clip.ts`
   ...
   math so CPU and GPU apply it identically (gated by `qkClipTrajectoryParity` in `gpu-parity.ts`
   ...
   (T=128).
4. **Aux-group GPU residency**: done. `src/backend/adamw-gpu.ts` `AdamWGpu` runs AdamW entirely on
   ...
   dominated, and it grows with vocab·hidden, so this is the real win at scale). Gated by the existing
   GPU-vs-CPU trajectory-parity tests, which now exercise GPU AdamW and still hold within BWD
   tolerance.
5. **GGUF checkpoint loader**: resume own checkpoints, done. `src/export/load-gguf.ts` is the
   inverse of `export-gguf.ts`: `configFromGGUF()` rebuilds the `Gemma3Config` from `gemma3.*`
   ...
   `loadGemma3FromGGUF(bytes)` returns `{ model, cfg, tokenizer }` ready to sample or keep
   training. This is the resume path the curriculum stages chain through. `BPETokenizer.fromData()` rebuilds
...
src/backend/   wgsl.ts (pure WGSL kernel-source generators + codegen/dispatch helpers),
               ...
               f16.ts (host f16<->f32 for mixed precision), train-gpu.ts,
               muon-gpu.ts + adamw-gpu.ts (GPU-resident optimizers)
src/export/    export_gguf (model -> GGUF, the llama.cpp contract), load_gguf (GGUF -> model)
tests/         gradcheck.ts (FD gradient gate), gpu-parity.ts (GPU-vs-CPU gate)
...
roadmap, and `.github/contributing.md` for the working rules.
