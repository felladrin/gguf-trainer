...
> **Premise check (2026-08-12):** this doc assumes the target is a general chat assistant.
> `model-positioning.md` surveys what sub-100M models are actually used for and finds no production
> users for a small causal LM as such, which reprioritizes SFT toward classification/routing and
> reranking. DPO/GRPO stay valid, but for a _task_ model the preference data would be task
> preferences, not chat preferences.
...
| Load a second model (the frozen reference) from a GGUF              | `src/export/load-gguf.ts` `loadWeightsFromGGUF()` / `loadGemma3FromGGUF()` |
| Device-resident training loop                                       | `src/backend/train-gpu.ts` `trainLMGpuResident()`                          |
