...
import { gemma3, type Gemma3Config, Gemma3Model } from "../src/arch/gemma3.ts";
import { wsdSchedule } from "../src/train/schedule.ts";
import { applyQKClip, qkLogitScale } from "../src/train/qk-clip.ts";
...
async function main() {
  ...
  }
  {
    ...
    if (!ok) failures++;
    console.log(
      `  ${ok ? "ok " : "FAIL"} ${"target vocab range".padEnd(24)}        ` +
        `V=${V}, ${cases.length} cases${bad.length ? `, failed: ${bad.join(", ")}` : ""}`,
    );
  }
  {
  ...
  }
  {
    ...
    // normalized teacher row (mass 1), a truncated top-k row (mass < 1, where
    // the gradient is S·p − q rather than p − q), and an ignored row (first id
    // -1: no loss, no gradient, out of the mean).
    const T = 4, V = 9, K = 3;
    const logits = randTensor([T, V], rng);
    const ids = [
      1,
      ...
      2,
      2,
      7, // duplicate id: the two −q writes must both land
    ];
    const q = [
      0.5,
      0.3,
    ...
    fdCheck("softCrossEntropy", [logits], () => softCrossEntropy(logits, ids, q, K));
  }

  // Whole model, once per REGISTERED ARCHITECTURE: adding an architecture to the
  // registry automatically puts its forward pass under finite-difference check.
  for (const arch of ARCHITECTURES) {
    const cfg = arch.tinyConfig(13);
    const model = arch.build(cfg, mulberry32(99));
    const ids = [1, 5, 2, 9, 5];
    ...
    const fresh = arch.build(cfg, mulberry32(99));
    ...
    console.log(
      `  ${dirFails === 0 ? "ok " : "FAIL"} ${arch.name} @ real init (directional, 4 trials)`,
    );
  }
  ...
  // loadModelFromGGUF, and confirm config, tokenizer, weights, and the actual
  ...
  // stores without the f16 fallback: exercising the loader's dequant path.
  {
    ...
    for (const [quant, wtol, ltol] of [["f32", 0, 0], ["q8_0", 0.05, 0.05]] as const) {
      const bytes = gemma3.exportGGUF(model, tok.export(), cfg, { quant });
      const { model: m2, cfg: loaded, tokenizer: tok2 } = loadModelFromGGUF(bytes);
      const cfg2 = loaded as Gemma3Config;
      ...
      for (const k of Object.keys(cfg) as (keyof Gemma3Config)[]) {
        const a: unknown = cfg2[k], b: unknown = cfg[k];
  ...
  // llama.cpp keeps them in output, so both must recover as specials. The
  // dequantizer must also decode BF16 and reject unsupported (k-quant) types.
  {
    ...
    const bytes = gemma3.exportGGUF(model, tok.export(), cfg, { quant: "f16" });
    ...
    const { tokenizer: tok2 } = loadModelFromGGUF(bytes);
