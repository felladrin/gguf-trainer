...
// step on host arrays. See train-gpu.ts.
//
  ...
  if (maxBinding && bytes > maxBinding) {
    ...
    throw new Error(
      `GPU storage buffer of ${mb(bytes)} MiB exceeds this device's limit of ` +
        ...
        `seqLen×vocab logits buffer, which --loss-chunk 8192 removes (pretrain, ` +
        `finetune, eval-loss and eval-choice all take it). A vocab×hidden weight ` +
        `gradient survives that flag: lower seqLen/--maxSeq, reduce the tokenizer ` +
...
export class WebGPUBackend implements OpsBackend {
  ...
  softCrossEntropy(
    ...
    loss._backward = () => {
      ...
      this.curLabel = "softCrossEntropy";
      this.queue.writeBuffer(eo.grad, 0, loss.grad);
      // Dense (+S·p) then sparse (−q): consecutive dispatches in a pass are
      ...
      this.dispatch(
        srcSoftCeBwdQ(T, V, k),
        [idBuf, qBuf, eo.grad, divBuf, el.grad],
        ceilDiv(T, 64),
      );
    };
    return loss;
  ...
  // --- raw-buffer access for GPU-side optimizers (see muon-gpu.ts) ---------------
