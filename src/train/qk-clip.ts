 ...
 */
export function applyQKClip(model: LanguageModel, tau: number): number {
  if (!(tau > 0)) throw new Error("qk-clip tau must be > 0");
  const qk = model.qkNorms?.();
  if (!qk) return 0; // no QK-norm in this architecture: nothing to clip
  const hd = qk.headDim;
  let clipped = 0;
  for (const L of qk.pairs) {
