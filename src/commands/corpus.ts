...
// (~0.28 st/s), so we target a few-hundred-MB SLICE, not trillions of tokens. That is
// Phi's quality-over-quantity lever, not scale.
//
// Documents are shuffled and concatenated with the "<|endoftext|>" marker that
// `tokenize` splits on. Each part is kept under ~480 MB so the whole-file readers
// work unchanged (V8 caps a single string at ~512 MB); a multi-GB corpus is built
// as SEVERAL parts (-p1, -p2, …), each downloaded in one continuous pass (shards
// are fetched once; leftover docs roll into the next part). `tokenize --text`
// accepts the whole part list and emits one token file.
//
//   deno run -A cli.ts corpus --size-mb 400 --out corpus/blend.txt --parts 1 --source id:config:field
//   deno run -A cli.ts corpus --smoke                     # tiny end-to-end self-test
//
// --size-mb is PER PART. With --parts 1 the output is exactly --out; with more,
// the files are --out with -p1 … -pN inserted before the extension.
//
// The frozen shared tokenizer is trained by `tokenize` on the first ~32 MB of
...
// `pretrain` (see --inject), tracked separately, not done here.

import { asyncBufferFromUrl, parquetReadObjects } from "hyparquet";
import { fetchParquetUrls, fetchRepoDataFiles } from "../data/hf.ts";
...
const DOC_SEP = "<|endoftext|>"; // boundary marker `tokenize` splits documents on
...
/** Rows per ranged read. Big enough to amortize the request, small enough that a
 * part's budget stops the transfer early. */
const ROW_BATCH = 20000;

/** Stream one parquet shard by HTTP RANGE, a row batch at a time, pulling only
 * the text column. Whole-shard downloads do not scale: FineWeb-Edu ships 2.15 GB
 * shards, which both waste bandwidth (a 480 MB part needs a fraction of one) and
 * overflow the single-buffer read the old path used. */
async function* parquetRowStream(url: string, field: string): AsyncGenerator<Row> {
  const file = await asyncBufferFromUrl({ url });
  for (let start = 0;; start += ROW_BATCH) {
    const rows = await parquetReadObjects({
      file,
      columns: [field],
      rowStart: start,
      rowEnd: start + ROW_BATCH,
    }) as Row[];
    if (!rows.length) return;
    for (const row of rows) yield row;
    if (rows.length < ROW_BATCH) return; // last batch of the shard
  }
}
...
async function* docStream(src: Source): AsyncGenerator<string> {
  ...
  for (let s = 0; s < urls.length; s++) {
    ...
    let rows: AsyncGenerator<Row> | Row[];
    if (urls[s].includes(".parquet")) {
      rows = parquetRowStream(urls[s], src.field);
    } else {
      const resp = await fetch(urls[s]);
      if (!resp.ok) die(`fetch ${urls[s]} -> ${resp.status} ${resp.statusText}`);
      rows = await parseDataFile(urls[s], new Uint8Array(await resp.arrayBuffer()));
    }
  ...
  if (mb > V8_STRING_CAP_MB) {
    console.log(
      `  WARNING: ${mb.toFixed(0)} MB exceeds the ~${V8_STRING_CAP_MB} MB whole-file read cap; ` +
        `\`tokenize\` will fail to read it whole. Lower --size-mb, or raise --parts.`,
    );
  }
...
async function run(v: Values) {
  const smoke = v.bool("smoke");
  // --source replaces the whole blend with ONE dataset, for a continual-pretrain
  // slice where the point is to isolate that corpus's effect.
  const sourceArg = v.opt("source");
  const single = sourceArg
    ? (() => {
      const [id, config, field] = sourceArg.split(":");
      if (!id || !config || !field) die(`--source must be "id:config:field", got ${sourceArg}`);
      return [{ id, config, split: "train", field, weight: 1 }] as Source[];
    })()
    : null;
  const sources = smoke ? SMOKE : (single ?? BLEND);
  const targetMB = smoke ? 1 : v.num("size-mb");
  const outPath = v.opt("out") ??
    (smoke ? "corpus/pretrain-smoke.txt" : "corpus/pretrain-blend.txt");
  const seed = v.num("seed");
  const parts = smoke ? 2 : v.num("parts");
  if (targetMB <= 0) die(`--size-mb must be > 0, got ${targetMB}`);
  if (!Number.isInteger(parts) || parts < 1) {
    die(`--parts must be a positive integer, got ${parts}`);
  }
...
export const corpusCommand: Command = {
  name: "corpus",
  summary: "Download and shuffle a pretraining corpus from Hugging Face datasets.",
  details: `Writes one plain-text file (or several parts) of shuffled documents separated by
<|endoftext|>, which is what \`tokenize\` expects.

The default blend mirrors what open-weight models pretrain on: filtered web as the bulk,
plus code, math and synthetic textbooks. Use --source to pull from a single dataset
instead, which is the right shape for continual pretraining, where you want one corpus's
effect isolated.

--smoke runs a tiny end-to-end self-test (about 1 MB, two parts) to check network, parsing
and disk before starting a real multi-GB download.`,
  examples: [
    "corpus --smoke",
    "corpus --size-mb 400 --out corpus/blend.txt",
    "corpus --source HuggingFaceFW/fineweb-edu:sample-10BT:text --size-mb 450 --parts 5 --out corpus/fineweb.txt",
  ],
  flags: [
    {
      name: "out",
      ...
      placeholder: "PATH",
      describe:
        "output text file; with --parts > 1, the part number is inserted before the extension",
    },
    {
      name: "size-mb",
      type: "number",
      default: 400,
      describe: "target megabytes PER PART (keep under ~480, the trainer reads a part whole)",
    },
    {
      name: "parts",
      type: "number",
      default: 1,
      describe: "split the corpus into this many files",
    },
    {
      name: "source",
      type: "string",
      placeholder: "ID:CONFIG:FIELD",
      describe:
        "use one dataset instead of the default blend, e.g. HuggingFaceFW/fineweb-edu:sample-10BT:text",
    },
    {
      name: "seed",
      type: "number",
      default: 1234,
      describe: "shuffle seed; the same seed reproduces the same corpus",
    },
    {
      name: "smoke",
      type: "boolean",
      describe: "tiny self-test download instead of a real corpus",
    },
  ],
  run: run,
};
