...
Node 22.6+ and Bun 1.4 run the CPU reference path too, which is enough for the test suite (`deno task test:node` and `deno task test:bun`; CI runs both), though the CLI itself is Deno-only.
...
- **No CPU training, and that is a refusal rather than a gap.** The loop is there, and `gpu-parity` runs it against the GPU trainer every test to prove their trajectories match. It is far too slow to train on. Measured on one machine at matching shapes and step counts: 16 tokens/s against the GPU's 786 at 6M parameters, and 1.6 tokens/s at 32M, so CPU throughput falls faster than the parameter count rises, 5.3x the parameters costing 10x the time. Extrapolate that to a 596M model, even generously, and a step costs seconds per token: a 100k-token fine-tune runs into weeks, where the GPU does 10M tokens in about a day. A flag that accepts the run and then never finishes would be worse than the refusal. Making it genuinely useful means a threaded SIMD or WASM backend, which is a different project from exposing the reference loop, and wiring the existing loop in is not free either: the optimizer, the checkpoint sidecar and the `--recompute` guard are all device-specific today, and the parity probe would compare the CPU against itself. Numbers and the full list in [docs/optimization.md](docs/optimization.md) lever 36.
- **If a CPU is what you have**, use [`transformers`](https://huggingface.co/docs/transformers) with [`peft`](https://huggingface.co/docs/peft); there is a page on [training on CPU](https://huggingface.co/docs/transformers/en/perf_train_cpu). Measured on Qwen3-0.6B-Base at seq 512, batch 1, 10 threads: LoRA gives 118 tokens/s at 6.1 GB peak, a full fine-tune 82 tokens/s at 12.8 GB, so both fit 32 GB.
