...
**A GPU adapter.** `pretrain` and `finetune` refuse to start without one and will not fall back to the CPU; see Honest limits below for why, and for what to use instead if a CPU is all you have. `eval-loss`, `eval-choice` and `generate` do take `--cpu`.
