...
**A GPU adapter, and training will not fall back to the CPU.** `pretrain` and `finetune` refuse to start without one. The CPU backend exists and is a complete training loop, but it is the correctness oracle the GPU kernels are validated against every test run, not a path anyone should train on: measured on one machine at the same shape and step count, it does 15.8 tokens/s at 6M parameters against the GPU's 786, and 1.6 tokens/s at 32M. It gets worse with size rather than better, and it is single-threaded, so more cores do not help. `eval-loss`, `eval-choice` and `generate` do take `--cpu`, at those speeds.

If a CPU is what you have, [`transformers`](https://huggingface.co/docs/transformers) with [`peft`](https://huggingface.co/docs/peft) is the answer, and there is a page on [training on CPU](https://huggingface.co/docs/transformers/en/perf_train_cpu). Measured on Qwen3-0.6B-Base at seq 512, batch 1, 10 threads: LoRA gives 118 tokens/s at 6.1 GB peak, a full fine-tune 82 tokens/s at 12.8 GB.
...
Memory scales with `seq-len x batch`, and on this trainer that means tens of GB at the shapes a real run uses. `--recompute` is the cheapest lever: it replays each layer in backward rather than storing its activations, which took the activation term from ~12.1 GB to ~0.8 GB on a 293M `qwen3` at seq 2048 (a 4.6x cut in the whole pool, most of the remainder being weights and gradients), and on this host-bound step it came out faster rather than slower ([docs/optimization.md](docs/optimization.md) lever 20). `--lora-rank N` goes further where the optimizer state is what hurts, freezing the base and training rank-N adapters instead: measured 4495 MB of optimizer state down to 63 MB on the same model, for ~14% less throughput, and the exported GGUF is still an ordinary dense checkpoint because the adapters are merged in on the way out (lever 21). `--reclaim` frees each micro-batch's activations as it goes, which cut peak GPU memory 5.6x in the measured A/B (39.3 GB to 7.0 GB) for 23% less throughput: it is off by default, and it is what makes a long-context run fit on a small GPU. Measured both ways in [docs/optimization.md](docs/optimization.md) lever 3b.
...
- **No CPU training, and that is a refusal rather than a gap.** The loop is there and is exercised by every parity test; it is roughly four orders of magnitude short of useful at a real size, and a flag that accepts the run and then never finishes would be worse than the refusal. Extrapolating the measured 1.6 tokens/s at 32M linearly to a 596M model, which is generous since the curve is worse than linear, gives about 12 seconds per token: a 100k-token fine-tune is 13 days and 10M tokens is years, against 25.7 hours for the same 10M on the GPU. Making it genuinely useful means a threaded SIMD or WASM backend, which is a different project from exposing the reference loop. Numbers in [docs/optimization.md](docs/optimization.md) lever 36.
- Training keeps float master weights and quantizes at export, so you can't really train in Q4_0.
- Context length hits a WebGPU buffer limit before compute becomes the problem, and the buffer that binds is the logits tensor (`seq-len x vocab x 4` bytes), not attention: attention takes a flash path from `seq-len` 2048 up and allocates no `[heads, T, T]` buffer there. At a 32768 vocab, 8192 needs 1 GiB and works on an adapter that grants its full buffer size; one that falls back to the WebGPU default of 128 MiB stops at 1024. `--loss-chunk N` lifts that cap by streaming the readout and the loss in vocab chunks, at the cost of recomputing the readout matmul in backward. Training and both eval commands take it.
